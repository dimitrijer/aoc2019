...
(defn parse-segments
  ...
  (let [length (Integer/parseInt (apply str length))
        ...
        prev-steps (or (:steps (last segments)) 0)
        prev-length (or (:length (last segments) 0))

        segment {:dir (directions->dir direction)
                 :start start :end end
                 :length length
                 :steps (+ prev-steps prev-length)}]
...
(defn get-intersections
  ...
  (for [red-segment red-wire
    ...
    (let [intersection (if (= :v (:dir red-segment))
                         [(first (varinvar red-segment)) (first (varinvar blue-segment))]
                         [(first (varinvar blue-segment)) (first (varinvar red-segment))])

          steps (+ (:steps red-segment)
                   (:steps blue-segment)
                   (Math/abs (reduce + (mapv - intersection (:start red-segment))))
                   (Math/abs (reduce + (mapv - intersection (:start blue-segment)))))]
...
(defn puzzle1
  ...
  (let [[red-wire blue-wire] (map path->wire paths)]
    (apply min (map (comp manhattan :point) (get-intersections red-wire blue-wire)))))
...
(defn puzzle2
  ...
  (let [[red-wire blue-wire] (map path->wire paths)]
    (println red-wire)
    (println blue-wire)
    (apply min (map :steps (get-intersections red-wire blue-wire)))))
